...
GITHUB_USER = "manavprakash" # Updated based on your setup
soft_blue = "#AEC6CF" 
...
<style>
    ...
    .stButton>button {{ background-color: V_BTN !important; color: V_TXT !important; border: 1px solid V_BLUE !important; border-radius: 12px !important; margin: 5px auto !important; width: 100%; }}
...
if st.session_state.current_page == "Journal":
    ...
    for i, label in enumerate(mood_labels):
        with mood_cols[i]:
            if st.button(label, key=f"mood_{i}"):
                ...
                st.session_state.private_journal.append({"time": datetime.now().strftime("%H:%M"), "diary": f"[{label} Energy]", "ai": f"Acknowledging that {label.lower()} energy. Breathe with me."})
                st.rerun()
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Consult Guide"):
            ...
            if audio_file:
                ...
                prompt = "Transcribe what you hear starting with 'You said: '. Then give a calm 1-paragraph response."
            else:
    ...
    if st.session_state.private_journal:
        ...
        if st.button("🔊 Play Mentor's Voice"):
...
# --- 8. MARKETPLACE PAGE (Restored & Centered) ---
elif st.session_state.current_page == "Marketplace":
    st.write("### ✨ Grounding Objects")
    m1, m2 = st.columns(2)
    with m1:
        st.markdown(f"<div class='mkt-box'><h4>Starter Ritual</h4><p>Stones, Buddha & Beads<br>₹2,499</p><a href='https://wa.me/{MY_PHONE}?text=Starter' class='wa-link'>Order via WhatsApp</a></div>", unsafe_allow_html=True)
    with m2:
        st.markdown(f"<div class='mkt-box'><h4>Master Sanctuary</h4><p>Complete 5-Item Set<br>₹4,999</p><a href='https://wa.me/{MY_PHONE}?text=Master' class='wa-link'>Order via WhatsApp</a></div>", unsafe_allow_html=True)
...
# --- 9. VISION ---
    ...
    st.markdown(f"<br><a href='https://wa.me/{MY_PHONE}?text=Vision' class='wa-link' style='width:200px; margin: 0 auto;'>Connect</a>", unsafe_allow_html=True)
