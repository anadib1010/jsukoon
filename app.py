...
st.markdown(f"""
    ...
    .stButton>button {{ 
        ...
        min-height: 40px !important; width: 100% !important; font-size: 12px !important; 
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.05), 0 2px 4px rgba(0,0,0,0.3) !important;
    }}
    
    /* RITUAL PROMPT STYLE */
    .ritual-prompt {{ font-size: 13px; font-style: italic; color: {soft_blue}; opacity: 0.8; text-align: center; margin-bottom: 15px; letter-spacing: 1px; }}
...
if st.session_state.current_page == "Journal":
    ...
    # RITUAL INFLUENCE: Physical Grounding Prompt
    st.markdown("<div style='height: 20px;'></div>", unsafe_allow_html=True)
    prompts = ["Hold your grounding stone. Feel its cool surface.", "Thread your fingers through your beads.", "Focus your gaze on the center of your Yantra.", "Rest your hand on a physical object nearby."]
    current_ritual = prompts[datetime.now().second % len(prompts)]
    st.markdown(f"<div class='ritual-prompt'>— {current_ritual} —</div>", unsafe_allow_html=True)

    audio_in = st.audio_input("Voice Note")
    text_in = st.text_area("Record reflection...", height=100)
    
    # CONSULT GUIDE (AI with Sensory Knowledge)
    if st.button("CONSULT GUIDE", use_container_width=True):
        if model:
            with st.spinner("Refining..."):
                ritual_context = "You are a Sukoon Mentor. Encourage the user to stay connected to their physical beads or stones. Keep responses short and calming."
                resp = model.generate_content([ritual_context, text_in if text_in else "I am seeking silence."]).text
                st.session_state.private_journal.append({"time": datetime.now().strftime("%H:%M"), "ai": resp})
                st.rerun()

    # ENERGY TRACKING (Below Consult)
    ...
    # ENERGY VISUALIZATION (Step 3: Tracking Influence)
    if st.session_state.energy_history:
        cols = st.columns(len(st.session_state.energy_history))
        ...
        history_html = "".join([f"<span class='energy-dot' title='{val}'></span>" for val in st.session_state.energy_history])
        st.markdown(f"<div style='text-align:center;'>{history_html}</div>", unsafe_allow_html=True)

    for entry in reversed(st.session_state.private_journal):
        st.info(f"{entry['time']} | {entry['ai']}")

elif st.session_state.current_page == "Market":
    st.markdown("<div class='section-header'>OBJECTS FOR GROUNDING</div>", unsafe_allow_html=True)
    st.write("These items are designed to work in harmony with the app's rituals.")
    st.write("**Grounding Stone:** For focus during chaos.  \n**Meditation Beads:** For rhythmic breath counting.")
    st.markdown(f"<a href='https://wa.me/{MY_PHONE}' style='color:{soft_blue};'>WhatsApp for Inquiries</a>", unsafe_allow_html=True)
...
st.markdown("<div class='footer-text'>Sukoon: A Digital & Physical Sanctuary.</div>", unsafe_allow_html=True)
